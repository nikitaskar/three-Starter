...
class createApp {
	constructor(opt) {
		...
		this.mesh.position.x = 30 * Math.cos( theta ) * Math.sin( phi );
		this.mesh.position.y = -30 * Math.cos( phi );
		this.mesh.position.z = 30 * Math.sin( theta ) * Math.sin( phi )
		...
		line.setGeometry( geometry, function( p ) { return 1*Math.pow( 4 * p * ( 1 - p ), 1 ); } ); // makes width sinusoidal 
	...
	animate() {
		...
		if (!this.anim) {
			this.anim = true;
			console.log(gsap)
			gsap.TweenLite.to(this.line.material.uniforms.visibility,5, {value:1})
		}
